...
//    C.DrawTile(material'UTCompLogo',(512*0.75)*Scale,(128*0.75)*Scale,0,0,256,64);
     C.DrawTile(material'UTCompLogo',(512*0.5)*Scale,(128*0.5)*Scale,0,0,256,64);
...
simulated event UpdateScoreBoard(Canvas C)
{
    ...
    minTiles=MAXPLAYERS; // was maxtiles, not sure why we need this... pooty.
    ...
    if(GRI.bTeamGame)
    {
        //DrawTeamInfoBox(C,0.02,0.12,1,MyScale,Min(RedPlayerCount, minTiles));  // RedTeam
        //DrawTeamInfoBox(C,0.514,0.12,0,MyScale,Min(BluePlayerCount, minTiles)); // BlueTeam
        DrawTeamInfoBox(C,0.02,0.07,1,MyScale,Min(RedPlayerCount, minTiles));  // RedTeam
        DrawTeamInfoBox(C,0.514,0.07,0,MyScale,Min(BluePlayerCount, minTiles)); // BlueTeam
        
    }
    else
    {
        //DrawTeamInfoBox(C,0.252, 0.12, 2,MyScale, Min(RedPlayerCount, minTiles)); // Deathmatch Team
        DrawTeamInfoBox(C,0.252, 0.07, 2,MyScale, Min(RedPlayerCount, minTiles)); // Deathmatch Team
    }
    C.SetDrawColor(255,255,255,255);
