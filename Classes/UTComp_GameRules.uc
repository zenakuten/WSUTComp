        ...
        if(UTCompMutator.EnableHitSoundsMode>0)
        {
            ...
            for(C=Level.ControllerList; C!=None; C=C.NextController)
            {
                if(BS_xPlayer(C)!=None && C.PlayerReplicationInfo!=None && (C.PlayerReplicationInfo.bOnlySpectator || C.PlayerReplicationInfo.bOutOfLives) && PlayerController(C).ViewTarget == InstigatedBy)
                {
                    ...
                    IsGroupedDamageType = BS_xPlayer(C).IsGroupedDamageType(DamageType);
...
{
    if(UTCompMutator.WarmupInfo!=None && UTCompMutator.WarmupInfo.bInWarmup)
        return false;
...
function UpdateClock(float F)
{
    ...
    if(UTCompMutator!=None && UTCompMutator.WarmupInfo!=None)
    {
        UTCompMutator.WarmupInfo.SetClientTimerOnly(int(Round(F)));
        if(bFirstEndOT && F<=0.0)
        {
            UTCompMutator.WarmupInfo.SetEndTimeOnly(int(Round(F)));
            bFirstEndOt=False;
        }
    }
}
